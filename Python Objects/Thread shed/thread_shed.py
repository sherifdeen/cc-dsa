...
for each_trans in daily_transactions_split:
...
for color in thread_sold:
  if not '&' in color:
    ...
    amp_split = []
    amp_split.append(color.split('&'))
    for color_grp in amp_split:
      for each_color in color_grp:
        thread_sold_split.append(each_color)
        
def color_count(color):
  color_counter = 0
  for a_color in thread_sold_split:
    if a_color == color:
      color_counter += 1
...
for a_color in colors:
  print("{} {} color threads were sold today".format(color_count(a_color), a_color))
